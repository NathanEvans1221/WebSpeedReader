...
- 🎯 **聪明選題 (Smart Extraction)**：內建 `Readability.js` 技術，自動過濾網頁廣告、導航欄及雜訊，精準抓取主體文字。
...
- 🤖 **多模型自由切換**：提供多款 Groq 高效能模型（GPT-OSS, Llama 3.3, Llama 3.1 等）供您選擇。
- 🌐 **多語系支援**：支援 7 種語言（繁中、英、日、韓、法、德、西），跨國資訊閱讀零障礙。
- 🎭 **多種總結風格**：支援簡明、標準、深度三種模式，另提供「自定義指令」讓您自由發揮。
- 🔊 **語音朗讀 (TTS)**：支援將總結內容朗讀出來，並可自動切換語言。
- 🌓 **主題與風格自訂**：內建深色模式，支援自定義字體顏色與背景色。
- 📜 **歷史紀錄管理**：自動保存最近 10 次總結，支援離線回看、單筆刪除、清空與匯出 JSON。
- 🖌️ **右鍵選單功能**：選取網頁一段文字後，點擊右鍵即可「總結選取內容」。
- 📊 **閱讀量節省統計**：顯示原字數 vs 總結字數，直觀展示效率提升。
...
> [!WARNING]
> **Windows 用戶請注意**：若載入時發生錯誤，請檢查並刪除目錄下的 `desktop.ini` 檔案（此為系統隱藏檔），因為它會導致 Chrome 報錯。
